...
struct NoBuddyWidget: Widget {
    ...
    var body: some WidgetConfiguration {
        ...
        return StaticConfiguration(kind: kind, provider: NotionDatabaseProvider()) { entry in
...
struct NotionDatabaseProvider: TimelineProvider {
    init() {
    ...
    func placeholder(in context: Context) -> NotionDatabaseEntry {
        ...
        return NotionDatabaseEntry(date: Date(), tasks: [], isLoading: true)
    }

    func getSnapshot(in context: Context, completion: @escaping (NotionDatabaseEntry) -> ()) {
        print("[NotionDatabaseProvider] getSnapshot called")
        Task {
            let tasks = await loadTasksFromCache()
            print("[NotionDatabaseProvider] getSnapshot loaded \(tasks.count) tasks")
            let entry = NotionDatabaseEntry(date: Date(), tasks: tasks, isLoading: false)
            completion(entry)
        }
    }

    func getTimeline(in context: Context, completion: @escaping (Timeline<NotionDatabaseEntry>) -> ()) {
        print("[NotionDatabaseProvider] getTimeline called")
        Task {
            let tasks = await loadTasksFromCache()
            print("[NotionDatabaseProvider] getTimeline loaded \(tasks.count) tasks")
            ...
            for hourOffset in 0 ..< 5 {
                ...
                let entry = NotionDatabaseEntry(date: entryDate, tasks: tasks, isLoading: false)
    ...
    private func loadTasksFromCache() async -> [TaskItem] {
        print("[NotionDatabaseProvider] loadTasksFromCache called")
        ...
        if selectedDatabases.isEmpty {
            print("[NotionDatabaseProvider] No databases selected, returning empty array")
        ...
        do {
            ...
            let taskItems = widgetTasks.map { widgetTask in
                TaskItem(
                    ...
                    projectName: selectedDatabases.first?.name ?? "Database"
                )
            }
...
#Preview(as: .systemMedium) {
    ...
    NotionDatabaseEntry(date: Date(), tasks: previewTasks, isLoading: false)
    NotionDatabaseEntry(date: Date().addingTimeInterval(300), tasks: [], isLoading: false)
}
